  ...
  <div class="flex justify-center items-center w-screen h-screen overflow-hidden">
    <div class="rounded-xl bg-white flex gap-10 p-4 m-3 absolute shadow-lg">
      <div class="rounded-xl relative">
        <img src="/images/bg-sidebar-desktop.svg" alt="side bar desktop" class="w-[18.9rem]">
      ...
      <div class="w-[42rem] px-20 flex flex-col" v-if="currentStep === 1">
        <div class="mt-10">
      ...
      <div class="w-[42rem] px-20 flex flex-col" v-else-if="currentStep === 2">
        <div class="mt-10">
      ...
      <div class="w-[42rem] px-20 flex flex-col" v-else-if="currentStep === 3">
        <div class="mt-10">
      ...
      <div class="w-[42rem] px-20 flex flex-col" v-else-if="currentStep === 4">
        <div class="mt-10">
          ...
          <div class="px-7 py-5 bg-magnolia rounded-xl mt-8">
            <div class="flex justify-between items-center">
              ...
              <span class="font-bold text-marine-blue">${{ billing === 'monthly' ? planSelected.price + '/mo' : planSelected.price * 10 + '/yr' }}</span>
            </div>
            <hr class="my-5">
            <div class="flex flex-col gap-5">
              <div class="flex justify-between items-center" v-if="isOnlineServiceSelected">
                ...
                <span class="text-marine-blue">+${{ billing === 'monthly' ? 1 + '/mo' : 10 + '/yr' }}</span>
              </div>
              <div class="flex justify-between items-center"  v-if="isLargerStorageSelected">
                <span class="text-cool-gray">Larger storage</span>
                <span class="text-marine-blue">+${{ billing === 'monthly' ? 2 + '/mo' : 20 + '/yr' }}</span>
              </div>
      ...
      <div class="w-[42rem] px-20 flex flex-col justify-center gap-7 text-center items-center" v-else-if="currentStep === 5">
...
const isValidEmail = () => formData.value.email !== '';
const isValidPhoneNumber = () => formData.value.phoneNumber !== '';

function isFirstStepValid(){
  let valid = true;
  let errorString = "This field is required.";
  if(!isValidName()) {
    errors.value.name = errorString;
    valid = false;
  } else {
    errors.value.name = "";
  }
  ...
  if(!isValidEmail()) {
    errors.value.email = errorString;
    valid = false;
  } else {
    errors.value.email = "";
  }
  ...
  if(!isValidPhoneNumber()) {
    errors.value.phoneNumber = errorString;
    valid = false;
  } else {
